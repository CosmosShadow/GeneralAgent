...
def test_write_novel():
    ...
    topic = "小白兔吃糖不刷牙的故事"
    summary = agent.run(
        f"小说的名称和主题是: {topic}，扩展和完善一下小说概要。要求具备文艺性、教育性、娱乐性。"
    )
    chapters = agent.run(
        "输出小说的章节名称和每个章节的概要，返回列表 [(chapter_title, chapter_summary), ....]",
        return_type=list,
    )
    contents = []
...
def test_multi_agents():
    ...
    story_writer = Agent(
        "你是一个故事创作家，根据大纲要求或者故事梗概，返回一个更加详细的故事内容。"
    )
    humor_enhancer = Agent(
        "你是一个润色作家，将一个故事进行诙谐润色，增加幽默元素。直接输出润色后的故事"
    )
...
def test_add_knowledge_files():
    ...
    agent = Agent(
        "你是AI助手，用中文回复。", workspace=workspace, knowledge_files=files
    )
...
def test_image_input():
    ...
    response = agent.user_input(
        ["What animal in the picture?", {"image": "test/data/test.jpeg"}]
    )
